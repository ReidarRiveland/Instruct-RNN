...
def make_data(task_dict = default_task_dict, batch_size = 128, num_batches = 500, holdouts=[]):
    ...
    pooler.map(populate, range(num_batches))
...
batch_size = 128
num_batches = 500
task_dict = default_task_dict
holdouts=[]



def edit_make_data(task_dict = default_task_dict, batch_size = 128, num_batches = 500, holdouts=[]):
    """Creates trial data
    Args:      
        task_dict (dict): dictionary where keys are tasks type and values are probabilities of the task occuring in the training set, 
        probabilities must add up to one, default is equal probabilities
...
for task in ['Go']: 
    ...
    print(task_file)
    input_data, target_data, masks_data, target_dirs, trial_indices = edit_make_data(holdouts=[task], batch_size=128, num_batches=1200)
