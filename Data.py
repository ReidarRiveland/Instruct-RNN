...
#     data = construct_batch(task, 8000)
#     np.save('training_data/' + task_file + '_training_data', data)
...
#     np.save('training_data/' + task_file+'/input_data', input_data)
#     np.save('training_data/' + task_file+'/target_data', target_data)
#     np.save('training_data/' + task_file+'/masks_data', masks_data)
#     np.save('training_data/' + task_file+'/target_dirs', target_dirs)
#     np.save('training_data/' + task_file+'/type_indices', trial_indices)
...
#         self.memmap_masks = np.lib.format.open_memmap(self.data_folder+'/masks_data.npy', dtype = 'int', mode = 'r', shape = (self.num_batches, self.batch_len, 120, 33))
...
class data_streamer(): 
    def __init__(self, batch_len, num_batches, task_dict = default_task_dict, holdouts=[]): 
        ...
        self.batches_per_task = int(np.ceil(num_batches/len(task_dict.keys())))
        self.trial_types = (list(task_dict.keys())*self.batches_per_task)[:num_batches]
