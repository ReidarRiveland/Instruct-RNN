...
#     fig.text(0.0, 0.5, r'$\textbf{Fraction Correct}$', va='center', rotation='vertical')
...
def plot_avg_curves(model_dict, foldername, smoothing = 1, plot_background_traces=False): 
    ...
    for holdout_task in task_list: 
        ...
        for model_type in model_dict.keys():       
            ...
            if plot_background_traces:
                smoothed_perf = gaussian_filter1d(train_data, sigma=smoothing)
                ax.plot(smoothed_perf, color = cog.ALL_STYLE_DICT[model_type][0])
    for model_type in model_dict.keys(): 
        train_data = avg_perf_dict[model_type]/len(task_list)
        smoothed_perf = gaussian_filter1d(train_data, sigma=smoothing)
        ax.plot(smoothed_perf, color = cog.ALL_STYLE_DICT[model_type][0], alpha=1, markersize=5, markevery=2)
        color = cog.ALL_STYLE_DICT[model_type][0]
...
#     cog = CogModule(model_dict)
#     fig, axn = plt.subplots(1,4, sharey = True, sharex=True, figsize =(16, 4))
#     legend_loc = (1.1, 0.7)
