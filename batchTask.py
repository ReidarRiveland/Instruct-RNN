...
class Task():
    TASK_LIST = ['Go', 'RT Go', 'Anti Go', 'Anti RT Go', 'DM', 'MultiDM', 'DMS', 'DNMS', 'DMC', 'DNMC']
    STIM_DIM = 32
    ...
    def __init__(self, num_trials): 
        ...
        for i in range(num_trials):
            T_go = (int(self.TRIAL_LEN - np.floor(np.random.choice([400, 500])/self.DELTA_T)), self.TRIAL_LEN)
            T_stim2 = (int(T_go[0]-np.floor(np.random.uniform(400, 600)/self.DELTA_T)), T_go[0])
            T_delay = (int(T_stim2[0]-np.floor(np.random.uniform(400, 600)/self.DELTA_T)), T_stim2[0])
            T_stim1 = (int(T_delay[0]-np.floor(np.random.uniform(400, 600)/self.DELTA_T)), T_delay[0])
            T_fix = (0, T_stim1[0])
    ...
    def _draw_ortho_dirs(self): 
        ...
        dir2 = np.random.uniform(dir1+(np.pi/5), dir1 + (2*np.pi - np.pi/5)%(2*np.pi))
        return (dir1, dir2)
...
class Match(Task): 
    def __init__(self, task_type, num_trials): 
        ...
        for i in range(num_trials): 
            ...
            else: direction2 = np.random.uniform(direction1+(np.pi/5), direction1 + (2*np.pi - np.pi/5)%(2*np.pi))
...
class DM(Task): 
    def __init__(self, task_type, num_trials):
        super().__init__(num_trials)
        assert task_type in ['DM', 'MultiDM'], "entered invalid task_type: %r" %task_type
        self.task_type = task_type
...
def construct_batch(task_type, num):
    ...
    if task_type == 'DMS': 
        trial = Match('DMS', num)
    if task_type == 'DNMS': 
        trial = Match('DNMS', num)
    if task_type == 'DMC': 
        trial = Match('DMC', num)
    if task_type == 'DNMC': 
        trial = Match('DNMC', num)
    return trial 
